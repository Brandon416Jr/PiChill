...
<head>
    ...
    <link rel="stylesheet" href="<%=request.getContextPath()%>/generaluser/css2/index3.css">
...
<body>
    ...
    <div class="main_content">
        ...
        <main class="main">
            ...
            <form method="post" action="<%=request.getContextPath()%>/contactUs.do" 
            name="action" enctype="multipart/form-data">
        
                <div>
                    <label for="formType" class="questiontype">問題類別 : </label><br>
                    <input type="radio" id="announcement" name="questionType" value="公告" required>
                    <label for="announcement" class="announcement">公告</label>&nbsp&nbsp&nbsp
                    <input type="radio" id="other" name="questionType" value="其他" checked required>
                    <label for="other" class="question">一般問題</label>
                </div><br>

                <div>
                    <label for="subject" class="title">主旨 : </label>
                    <input type="text" id="subject" name="getformPurpose" value="<%=(contactUs == null) ? "廁所的燈壞了" : contactUs.getformPurpose()%>"
                    size="45" required>
                </div><br>
            ...
            <!-------- 送出按鈕  ------->
              <input type="hidden" name="action" value="insert" style="width:150px; height:44px;"> 
  			  <input type="submit" value="insert" id="next">
            ...
            $('#imageUpload').on('change', function (e) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    reader.onload = function (event) {
                        $('#imagePreviews').append('<div class="image-preview"><img src="' + event.target.result + '"><button class="delete-image">刪除</button></div>');
                    }
            ...
            $('#contactForm').button(function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        console.log('表單已成功提交！伺服器回應：', response);
                        // 在此處添加成功提交後的處理方式，例如顯示成功訊息或重置表單
                    },
                    error: function (error) {
                        console.error('表單提交失敗！錯誤訊息：', error);
