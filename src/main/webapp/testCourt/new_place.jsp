        ...
        <main class="main">
            <!-- <div class="row g-3"> -->
				...
				<%-- 錯誤表列 --%>
        			...
        			<br>
			<form action="${pageContext.request.contextPath }/place/placef.do"
			      METHOD="post"	>
...
<%-- 						<input type="text" name="courtID" value="<%=(place == null) ? "" : place.getCourt().getCourtID()%>"  --%>
<!-- 					       placeholder="如A、B"> -->
<!-- 					</div> -->
<!-- 					<br><br> -->
						...
						<div class="form-check">
						  <div class="radio" >
							<select name="ball" style="position:relative;left: 20px;">
							  <option value="0"	${(place.ball==0)? 'selected': ''}>籃球</option>
							  <option value="1" ${(place.ball==1)? 'selected': ''}>排球</option>
							  <option value="2" ${(place.ball==2)? 'selected': ''}>羽球</option>
							</select>
						  </div>
						</div>
					</div>
					...
					<input type="text" id="text-input" name="placeName"	
					       value="<%=(place == null) ? "" : place.getPlaceName()%>" 
					...
					value="<%=(place == null) ? "" : place.getPlaceFee()%>" class="form-control" />
