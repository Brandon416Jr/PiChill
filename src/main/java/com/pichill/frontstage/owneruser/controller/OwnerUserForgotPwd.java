...
@WebServlet("/ouser/forgotpwd")
public class OwnerUserForgotPwd extends HttpServlet {
...
//		}
		...
		if ("forgot".equals(action)) {
			...
			if (oEmail == null || oEmail.trim().length() == 0) {
				errorMsgs.put("oEmail", "信箱請勿空白");
			} else if (!oEmail.trim().matches(oEmailReg)) {
				errorMsgs.put("oEmail", "信箱格式不符");
			}

			// Send the use back to the form, if there were errors
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req.getRequestDispatcher("/login/oLogin/oUserForgotPwd.jsp");
				failureView.forward(req, res);
				return;// 程式中斷
			}
			/*************************** 2.開始查詢資料 *****************************************/
			OwnerUserServiceFront oUserSvcF = new OwnerUserServiceFront();
			OwnerUser ownerUser = oUserSvcF.getOwnerUserByoEmail(oEmail);
...
//			req.setAttribute("isForgotStepOneCompleted", true);
			...
			String email = String.valueOf(oEmail);
...
//			req.setAttribute("isForgotStepOneCompleted", true);
			...
			String verificationValue = jedis.get(oEmail);
...
//					req.setAttribute("isForgotStepOneCompleted", false);
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/login/oLogin/oUserChangePwd.jsp");
					failureView.forward(req, res);
		...
		if ("changePassword".equals(action)) {
			...
			if (oPassword == null || oPassword.trim().length() == 0) {
				errorMsgs.put("oPassword", "密碼請勿空白");
			} else if (!oPassword.trim().matches(oPasswordReg)) {
			...
			} else if (!confirmPass.equals(oPassword)) {
...
//				req.setAttribute("isForgotStepOneCompleted", false);
			...
			/*************************** 2.開始修改密碼 ***************************************/
			String oEmail = req.getParameter("oEmail");
			String hashedPassword = BCrypt.hashpw(oPassword, BCrypt.gensalt());
			...
			oUserSvcF.updateOwnerUser(ownerUser);

			req.setAttribute("ownerUser", ownerUser); // 資料庫update成功後,正確的的memberVO物件,存入req
			String url = "/login/oLogin/oUserLogin.jsp";
